...
function invoke_strength(invocation, state)
    ...
    elseif not state then
        console_print(invocation.owner.owner.name .. " no longer feels powerful.", color_player)
...
function invoke_regen(invocation, state)
    if state then
        ...
        console_print(invocation.owner.owner.name .. " feels a little better!", color_player)
    elseif not state then
        table.remove(invocation.owner.invocations, index_of(invocation))
    end
end
...
function invoke_vampirism(invocation, state)
    if state then
        ...
        invocation.owner.owner.colortext = G.newText(G.getFont(), {color_white, "w"})
